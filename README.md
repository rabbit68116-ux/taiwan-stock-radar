...
Taiwan Stock Radar v1.2 是一個面向台灣股票單股研判的研究型 AI skill，目標是讓 AI agent 能用更接近專業投資委員會的方式評估一檔股票。

> 一檔股票。六位 specialist AI analyst。一次正式研究會議。一份結構化決策文件。<br>
...
| 項目 | 說明 |
|---|---|
| 產品模式 Product mode | `v1.2` single-stock multi-agent council |
| 覆蓋範圍 Coverage | `TWSE / TPEX` 單股深度研判 |
| 輸出樣式 Output style | 專業會議摘要 + 決策文件 |
| 最終交付 Final deliverable | 趨勢預測、情境分支、買入區、停損區、停利區、失效條件 |
